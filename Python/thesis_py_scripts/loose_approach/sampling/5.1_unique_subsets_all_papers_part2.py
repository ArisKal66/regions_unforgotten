...
unique_strict_researchers = pd.read_csv('/data2/aris/metadata/aps-dataset-metadata-2021/loose_appr/96prc/unique_loose96prc_researchers.csv', sep=';') # Change

len_filtered = len(filtered_candidates)
# Target averages from LGBTQ dataset
target_avg_max_date_interval = unique_strict_researchers['max_date_interval'].mean()
target_avg_max_min_interval = unique_strict_researchers['max_min_interval'].mean()
print(f"targets// from last date of APS: {target_avg_max_date_interval} // career:{target_avg_max_min_interval}")

filtered_candidates = filtered_candidates[filtered_candidates['max_date_interval'] <= unique_strict_researchers['max_date_interval'].max()]
...
output_dir = '/data2/aris/metadata/aps-dataset-metadata-2021/loose_appr/96prc/96prc_subsets/all_papers' # Change
...
for it in range(iterations):
    ...
    tolerance_max_date = 1.8 # changed
    tolerance_max_min = 2.5 # changed
    ...
    for i in range(num_samples):
        ...
        for attempt in range(20000):  # Limit attempts to avoid infinite loops

            try:
                sampled_authors = remaining_authors.sample(
                    n=len(unique_strict_researchers),  # Change
            ...
            if attempt % 4000 == 0 and attempt > 0:
        ...
        output_file_path = os.path.join(output_dir, f'Iter_{it + 1}_APS_subset_{i + 1}_all_papers.csv')
...
ct = datetime.datetime.now() # Current time
print(f"Process Completed! Total time taken: {total_elapsed_time:.2f} seconds, or {(total_elapsed_time/60):.2f} minutes. at {ct}")