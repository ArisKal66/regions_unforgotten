...
filtered_candidates = pd.read_csv('/data2/aris/metadata/aps-dataset-metadata-2021/loose_appr/96prc/filtered_candidates_96prc_all_papers.csv', sep=';') # Change
filtered_candidates = filtered_candidates.rename(columns={"author_name": "input_value"})

filtered_candidates_metadata = pd.read_csv('/data2/aris/metadata/aps-dataset-metadata-2021/loose_appr/96prc/filtered_candidates_96prc_metadata_all_papers.csv', sep=';') # Change
...
unique_strict_researchers_nonE = pd.read_csv('/data2/aris/metadata/aps-dataset-metadata-2021/loose_appr/96prc/unique_loose96prc_researchers.csv', sep=';') # Change
...
output_dir = '/data2/aris/metadata/aps-dataset-metadata-2021/loose_appr/96prc/loose_subsets_KMeans/all_papers/' # Change
os.makedirs(output_dir, exist_ok=True)

# Perform clustering
n_clusters = 8  # Number of clusters
clustering_features = filtered_candidates[['max_date_interval', 'max_min_interval']]
scaler = StandardScaler()  # Standardize features for better clustering
clustering_features_scaled = scaler.fit_transform(clustering_features)

np.random.seed(int(time.time()))
kmeans = KMeans(n_clusters=n_clusters, random_state=np.random.randint(0, 50))
filtered_candidates['cluster'] = kmeans.fit_predict(clustering_features_scaled)

# Get centroids and identify the closest cluster to the target metrics
centroids = scaler.inverse_transform(kmeans.cluster_centers_)  # Scale back to original values
distances_to_target = np.linalg.norm(centroids - np.array([target_avg_max_date_interval, target_avg_max_min_interval]), axis=1)
closest_cluster = np.argmin(distances_to_target)

print(f"Closest cluster to the targets: {closest_cluster} // Centroid of closest cluster: {centroids[closest_cluster]}")

# Filter candidates within the closest cluster
cluster_candidates = filtered_candidates[filtered_candidates['cluster'] == closest_cluster].copy()
...
for it in range(iterations):
    ...
    tolerance_max_min = 1.25 # changed
    ...
    for i in range(num_samples):
        ...
        remaining_authors = cluster_candidates[~cluster_candidates['input_value'].isin(used_authors)].copy() # Added .copy to avoid warning
        ...
        for attempt in range(40000):  # Limit attempts to avoid infinite loops
            ...
            if attempt % 8000 == 0 and attempt > 0:
                tolerance_max_date *= 1.12
                tolerance_max_min *= 1.055
        ...
        output_file_path = os.path.join(output_dir, f'Iter_{it + 1}_APS_subset_{i + 1}_all_papers.csv') # Change
