...
# Plot for max_min_interval on ax1
counts, bins, _ = ax1.hist(df_lgbt['max_min_interval'], bins=np.arange(0, df_lgbt['max_min_interval'].max() + 5, 5), alpha=0.5)
ax2.set_xlabel('Career (years)')
ax1.set_ylabel('Num. of Researchers')
ax1.set_title('Career Stage')
add_value_counts(ax1, counts, bins)

# Plot for max_min_interval on ax2
counts, bins, _ = ax2.hist(df_lgbt['max_min_interval'], bins=np.arange(0, df_lgbt['max_min_interval'].max() + 5, 5), alpha=0.5)
add_value_counts(ax2, counts, bins)

# Set y-limits for the broken axis
ax1.set_ylim(140, 150)  # Outliers only
