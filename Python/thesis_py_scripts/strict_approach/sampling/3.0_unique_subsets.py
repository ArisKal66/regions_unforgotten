...
APS_last_50yrs = APS_last_50yrs[APS_last_50yrs['five_point_class'].notna() & (APS_last_50yrs['five_point_class'] != 'C5')]


# Load strict paper IDs
strict_paper_ids = pd.read_csv('/data2/aris/metadata/aps-dataset-metadata-2021/strict_appr/strict_paper_ids_nonC5.csv', sep=';')

# Prepare the long format of authors
...
# Drop rows with empty author names
long_authors = long_authors[long_authors['author_name'] != '']

matches_to_exclude = pd.concat(
    [strict_paper_ids['match1'], strict_paper_ids['match2']]
).dropna()  # Combine match1 and match2, dropping NaN values

long_authors = long_authors[~long_authors['author_name'].isin(matches_to_exclude)]
log_time("Exclusion of strict paper IDs (match1 and match2) completed")

# Remove rows where author_name starts with special characters or numbers
# Special characters to filter: †, ‡, _, \, [, (, <, ., and any digit (0-9)
...
# Group data to calculate metrics
result_df = (
...
result_df['max_date_interval_nonC5'] = round(
    (pd.to_datetime('2021-12-30') - result_df['max_date']).dt.days / 365.242, 1
)
result_df['max_min_interval_nonC5'] = round(
...
result_df.to_csv(
    '/data2/aris/metadata/aps-dataset-metadata-2021/strict_appr/filtered_candidates.csv',
    sep=';',
    index=False
)
